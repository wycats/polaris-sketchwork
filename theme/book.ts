...
globalThis["playground_text"] = (playground: HTMLElement): string => {
...
(function codeSnippets() {
  ...
  if (playgrounds.length > 0) {
    fetch_with_timeout("https://play.rust-lang.org/meta/crates", {
      ...
      .then((response: { crates: { id: string }[] }) => {
  ...
  function handle_crate_list_update(
    playground_block: HTMLPreElement,
    playground_crates: string[]
  ) {
    ...
    if (window.ace) {
      const code_block = playground_block.querySelector("code") as HTMLElement;
      if (code_block.classList.contains("editable")) {
        ...
        editor.commands.addCommand({
          ...
          exec: (_editor: unknown) => run_rust_code(playground_block),
        });
      }
    }
  }
  ...
  function run_rust_code(code_block: HTMLElement): void {
    let result_block = code_block.querySelector(".result") as HTMLElement;
    if (!result_block) {
    ...
    const classes = (code_block.querySelector("code") as HTMLElement).classList;
    let edition: string = "2015";
  ...
  Array.from(document.querySelectorAll("code.language-rust")).forEach(function (
    ...
    // add expand button
    const pre_block = block.parentNode as Element;
    pre_block.insertBefore(buttonsContainer, pre_block.firstChild);

    const buttons = pre_block.querySelector(".buttons") as HTMLElement;
...
(function themes() {
  ...
  const themePopup = document.getElementById("theme-list") as HTMLUListElement;
  ...
  const stylesheets = {
    ayuHighlight: document.querySelector(
      "[href$='ayu-highlight.css']"
    ) as HTMLLinkElement,
    tomorrowNight: document.querySelector(
   ...
   * @returns {void}
   */
  function focus(selector: string, inside: ParentNode = document) {
  ...
  function set_theme(theme: string, store = true) {
    let ace_theme: string;
   ...
   */
  function focusIsIn(target: EventTarget | null, container: Element) {
    if (target === null) {
      return false;
    }
...
(function sidebar() {
  ...
  let firstContact: { x: number; time: number } | null = null;
  ...
  const sidebarAnchorToggles = document.querySelectorAll(
    "#sidebar a.toggle"
  ) as NodeListOf<HTMLAnchorElement>;
...
(function clipboard() {
  ...
  function showTooltip(target: EventTarget | null, msg: string) {
...
type ElementType<T extends Node> = abstract new (...args: any[]) => T;

type ElementForType<E extends ElementOptions<any>> = E extends ElementType<
  infer T
>
  ? T
  : E extends NullableElementOptions<infer T>
  ? T | null
  : E extends PresentElementOptions<infer T>
  ? T
  : never;

type ElementOptions<T extends Node = Node> =
  | ElementType<T>
  | NormalizedElementOptions<T>;

type NormalizedElementOptions<T extends Node> =
  | PresentElementOptions<T>
  | NullableElementOptions<T>;

type NormalizedFor<O extends ElementOptions> = O extends ElementType<any>
  ? {
      type: O;
      null: "reject";
    }
...
type PresentElementOptions<T extends Node> = {
  type: ElementType<T>;
  null: "reject";
};

type NullableElementOptions<T extends Node> = {
  type: ElementType<T>;
  null: "allow";
};

function normalize<O extends ElementOptions>(options: O): NormalizedFor<O> {
  if (typeof options === "function") {
    return { type: options, null: "reject" } as NormalizedFor<O>;
  } else {
    return options as NormalizedFor<O>;
  }
}

function hasType<E extends Element>(
  node: Node,
  type: ElementType<E>
): node is E {
...
function verified<T, O extends ElementOptions = ElementType<HTMLElement>>(
  node: EventTarget | null,
  as: (element: ElementForType<O>) => T,
  type?: O
): T {
  validateNode(node, type ?? HTMLElement);
  return as(node as ElementForType<O>);
}

function validateNode<O extends ElementOptions>(
  node: EventTarget | null,
  options: O
): ElementForType<O> {
  ...
  if (node === null) {
    if (normalized.null === "reject") {
      ...
      return null as ElementForType<O>;
    }
  }

  if (node instanceof normalized.type) {
    return node as ElementForType<O>;
  } else {
    throw new Error("Element is not of the expected type");
  }
}

function validate<O extends ElementOptions, N extends EventTarget>(
  node: N | null,
  get: (node: N extends Node ? N : Node) => Node | null,
  options: O
): ElementForType<O> {
  ...
  return validateNode(get(node as Extract<N, Node>), options);
}

function parentElement(node: EventTarget | null): HTMLElement;
function parentElement<O extends ElementOptions>(
  node: EventTarget | null,
  rawOptions: O
): ElementForType<O>;
function parentElement(
  node: EventTarget | null,
  rawOptions: ElementOptions = HTMLElement
): Element {
  return validate(node, (node) => node.parentNode, rawOptions) as Element;
}

function closest<O extends ElementOptions = ElementType<HTMLElement>>(
  node: EventTarget | null,
  selector: string,
  type?: O
): ElementForType<O> {
...
function firstChild<O extends ElementOptions = ElementType<Node>>(
  node: EventTarget | null,
  options?: O
): ElementForType<O> {
  return validate(
...
function find<T extends Element>(
  ...
  if (!(found instanceof type)) {
    throw new Error(
      `Found element with selector "${selector}" but it was not of type "${type.name}"`
    );
  }
...
function findAll<T extends Element>(
  ...
  return [...found].filter((elem): elem is T => elem instanceof type);
}

function contains(parent: ParentNode, child: EventTarget | null) {
  if (child === null) {
  ...
  if (!(child instanceof Element)) {
    throw new Error(
      "Unexpected: child parameter to contains was not an element"
    );
  }
